...
// Free registration endpoint for St. Joseph University students. No payment —
// deliberately separate from /api/audition/submit so the paid flow can never be
// bypassed. Writes to the SJU sheet (GOOGLE_SHEET_ID_SJU).
...
export async function POST(request: Request) {
  try {
    ...
    // Field validation (same required set as the paid flow)
    ...
    // Strip payment (free flow) and process to the SJU sheet.
    const data: AuditionData = { ...body, payment: undefined };
    await processFreeSubmission(data, apiKey, process.env.GOOGLE_SHEET_ID_SJU);

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("[/api/audition/submit-sju]", err);
    return NextResponse.json(
