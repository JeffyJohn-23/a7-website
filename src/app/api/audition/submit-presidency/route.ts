...
// Free registration endpoint for Presidency University students. No
// payment — deliberately separate from /api/audition/submit so the paid flow
// can never be bypassed. Writes to the Presidency sheet (GOOGLE_SHEET_ID_PRESIDENCY).
...
export async function POST(request: Request) {
  try {
    ...
    // Strip payment (free flow) and process to the Presidency sheet.
    const data: AuditionData = { ...body, payment: undefined };
    await processFreeSubmission(data, apiKey, {
      badge: "PRESIDENCY · FREE",
      subjectLabel: "Presidency University Registration",
      sheetId: process.env.GOOGLE_SHEET_ID_PRESIDENCY,
      sheetIdEnvVar: "GOOGLE_SHEET_ID_PRESIDENCY",
      includeRollNumber: true,
    });

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("[/api/audition/submit-presidency]", err);
    return NextResponse.json(
