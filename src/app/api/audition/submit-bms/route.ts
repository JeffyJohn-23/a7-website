...
// Free registration endpoint for BMS College students. No payment —
// deliberately separate from /api/audition/submit so the paid flow can never
// be bypassed. Writes to the BMS sheet (GOOGLE_SHEET_ID_BMS).
...
export async function POST(request: Request) {
  try {
    ...
    // Strip payment (free flow) and process to the BMS sheet.
    const data: AuditionData = { ...body, payment: undefined };
    await processFreeSubmission(data, apiKey, {
      badge: "BMS · FREE",
      subjectLabel: "BMS College Registration",
      sheetId: process.env.GOOGLE_SHEET_ID_BMS,
      sheetIdEnvVar: "GOOGLE_SHEET_ID_BMS",
      includeRollNumber: true,
    });

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("[/api/audition/submit-bms]", err);
    return NextResponse.json(
