...
// Free registration endpoint for BGS students. No payment — deliberately
// separate from /api/audition/submit so the paid flow can never be bypassed.
// Writes to the BGS sheet (GOOGLE_SHEET_ID_BGS).
...
export async function POST(request: Request) {
  try {
    ...
    // Strip payment (free flow) and process to the BGS sheet.
    const data: AuditionData = { ...body, payment: undefined };
    await processFreeSubmission(data, apiKey, {
      badge: "BGS · FREE",
      subjectLabel: "BGS Registration",
      sheetId: process.env.GOOGLE_SHEET_ID_BGS,
      sheetIdEnvVar: "GOOGLE_SHEET_ID_BGS",
      includeRollNumber: true,
    });

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("[/api/audition/submit-bgs]", err);
    return NextResponse.json(
