...
// Free registration endpoint for New Horizon students. No payment —
// deliberately separate from /api/audition/submit so the paid flow can never
// be bypassed. Writes to the New Horizon sheet (GOOGLE_SHEET_ID_NEWHORIZON).
...
export async function POST(request: Request) {
  try {
    ...
    // Strip payment (free flow) and process to the New Horizon sheet.
    const data: AuditionData = { ...body, payment: undefined };
    await processFreeSubmission(data, apiKey, {
      badge: "NEW HORIZON · FREE",
      subjectLabel: "New Horizon Registration",
      sheetId: process.env.GOOGLE_SHEET_ID_NEWHORIZON,
      sheetIdEnvVar: "GOOGLE_SHEET_ID_NEWHORIZON",
      includeRollNumber: true,
    });

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("[/api/audition/submit-newhorizon]", err);
    return NextResponse.json(
