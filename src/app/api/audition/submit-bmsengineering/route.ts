...
// Free registration endpoint for BMS Engineering students. No payment —
// deliberately separate from /api/audition/submit so the paid flow can never
// be bypassed. Writes to the BMS Engineering sheet (GOOGLE_SHEET_ID_BMSENGINEERING).
...
export async function POST(request: Request) {
  try {
    ...
    // Strip payment (free flow) and process to the BMS Engineering sheet.
    const data: AuditionData = { ...body, payment: undefined };
    await processFreeSubmission(data, apiKey, {
      badge: "BMS ENGINEERING · FREE",
      subjectLabel: "BMS Engineering Registration",
      sheetId: process.env.GOOGLE_SHEET_ID_BMSENGINEERING,
      sheetIdEnvVar: "GOOGLE_SHEET_ID_BMSENGINEERING",
      includeRollNumber: true,
    });

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("[/api/audition/submit-bmsengineering]", err);
    return NextResponse.json(
